...
label_25FD:
	...
	LD HL,(SEED)   		  ; HL = X_n  (current SEED from system variable)
 	LD E,$FD			  ; E = 253 = c
...
IMRND1:
	LD (SEED),HL  	      ; store X_(n+1) in SEED for next starting point.
						  ; X_(n+1) = (254×X_n + 253) mod 65537
                          ; equivalently: (254 × (X_n+1) mod 65537) - 1
	LD C,L
	LD B,H
	call	STK_TO_BC	  ; routine STACK-BC places on calculator stack
	rst	28h   		      ; FP-CALC 
	DEFB    $3D           ;re-stack, its exponent is therefore available.
				          ;Expoent Will Be 0 if number is ZERO
	DEFB    $38           ;end-calc
